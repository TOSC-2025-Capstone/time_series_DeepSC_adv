...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 40 == 0:
            ...
            sample_output = sample_output.contiguous().view(-1, 512, 8) # 비교용 결과본

            for idx in range(3):
                os.makedirs(save_fig_dir, exist_ok=True)
                # batch: [batch_size, window, feature]
                input_norm = orig_fixed_batch[idx:idx+1, :, :8].detach().cpu().numpy()  # [3, window, 6]
                output_norm = (
                    sample_output[idx:idx+1, :, :8].detach().cpu().numpy()
                )  # [6, window, 6]
