...
import pandas as pd
import pickle
from models.lstm_gru_models import (
    LSTMDeepSC,
    GRUDeepSC,
    Seq2SeqAttention,
    BiLSTMDeepSC,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_pbar):
            ...
            if epoch == 10 and batch_idx == 6:
                ...
                for i in range(input_norm.shape[1]):
                    plt.subplot(2, 3, i + 1)
                    plt.plot(
                        input_norm[:, i], label="Input (norm)", color="blue", alpha=0.7
                    )
                    plt.plot(
                        target_norm[:, i], label="target (norm)", color="red", alpha=0.7
                    )
                    plt.plot(
                        output_norm[:, i],
                        label="Output (norm)",
                    ...
                    plt.grid(True)
                plt.suptitle(
                    f"정규화 입력 vs Output (Epoch {epoch+1}, Batch {batch_idx+1})"
                )
                plt.tight_layout()
                plt.savefig(
                    f"model/train_input_vs_output_norm_epoch{epoch+1}_batch{batch_idx+1}.png",
                    dpi=200,
                )
                plt.show()
        ...
        if avg_val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(
                checkpoint_dir, f"{model_type}_deepsc_battery_epoch{epoch+1}.pth"
            )
